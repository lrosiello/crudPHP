...
class ValoracionesModel
{
    ...
    public function getValoracionById($id_valoracion)
    {
    ...
    public function editarValoracion($id_valoracion, $valoracion, $resena, $inadecuada)
    {
        if ($inadecuada == "true") {
            $inadecuada = 1;
        }

        $sentencia = $this->db->prepare('UPDATE valoracion SET valoracion=?, resena=?, inadecuada=? WHERE id=?');
        $sentencia->execute(array($valoracion, $resena, $inadecuada, $id_valoracion));
    }
    ...
    public function getResenasPorEmpresa($id_empresa)
    {
        $sentencia = $this->db->prepare("SELECT e.nombre AS nombre_empresa, v.resena AS resena_valoracion, v.valoracion AS valoracion_valoracion,
             v.id_usuario AS usuario_valoracion
            FROM valoracion v
            JOIN empresa e ON v.id_empresa = e.id
            WHERE v.id_empresa = ?
            ");
